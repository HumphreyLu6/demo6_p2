...
def move(distance=0.1, max_error=0.03, linear_scale=0.1):
    ...
    init_pose = odom
    while abs(init_pose.pose.pose.position.x - odom.pose.pose.position.x) < abs(distance) and abs(init_pose.pose.pose.position.y - odom.pose.pose.position.y) < abs(distance):
        out_twist = Twist()
